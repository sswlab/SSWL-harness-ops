...
3. **사용자 경로 직접 참조 금지**: 사용자가 제공한 코드 경로(`/home/user/...`, `~/research/...` 등)에서 직접 읽거나 실행하지 않는다. 반드시 `{작업경로}/inbox/`로 복사한 뒤 작업한다. 인벤토리, 모듈, 스킬 등 모든 산출물에서도 사용자의 원본 경로를 노출하지 않고, 작업 경로 기준 상대 경로를 사용한다
...
5. **스킬 간 충돌 방지**: 변환 결과물 내 스킬끼리 트리거/기능이 충돌하지 않도록 확인
