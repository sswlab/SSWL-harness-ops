...
사용자의 연구 코드를 참조할 때, **사용자의 로컬 경로를 그대로 사용하지 않는다.**

1. **작업 경로로 복사**: 사용자가 제공한 코드 파일은 `{작업경로}/code/` 하위로 복사한 뒤 참조한다. 원본 경로에서 직접 읽거나 수정하지 않는다
2. **논문 내 경로 노출 금지**: 논문 본문, 코드 스니펫, Figure 캡션 등에 사용자의 로컬 경로(`/home/user/...`, `~/research/...` 등)가 노출되지 않도록 한다
3. **코드 참조 시 상대 경로**: 논문이나 보고서에서 코드를 인용할 때는 작업 경로 기준 상대 경로 또는 파일명만 사용한다
